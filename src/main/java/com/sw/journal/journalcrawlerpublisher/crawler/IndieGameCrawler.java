...
//            // 카테고리 1개
//            String articleCategory = elem.select("div.the-post-header>div.post-meta>" +
//                    "div.post-meta-items>span.meta-item>a").text();
//            // 기사 제목
//            String articleTitle = elem.select("div.the-post-header>div.post-meta>h1").text();
//            // 기사 이미지
//            String imgUrl = elem.select("div.ts-row>div.post-content>figure>img").attr("src");
//            // 기사 내용
...
//        try {
//            Document doc = conn.get();
//            Elements elems = doc.select("div.post-meta.post-meta-a.has-below"); //set 자료형으로 관리해도 됨
//            //System.out.println(elems);
//            for (Element elem : elems) {
//                //System.out.println(elem.select("pubDate").text());
//                String link = elem.select("h2.is-title.post-title a").attr("href");   // 기사의 링크
//                String pubDate = elem.select("div.post-meta-items.meta-below span.meta-item.date span.date-link time.post-date").attr("datetime"); // 기사 발행 시간
